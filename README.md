...
Она добавляет пункт **«Мои плагины»** в настройки. Клик по нему открывает нативный UI магазина расширений Lampa, указывающий на наш `app/extensions.json`, — ставь / удаляй любой плагин оттуда. На первом запуске автоматически подтягивается `log-collector` (для сбора логов с телевизора).

Страница каталога: <https://vahagn-99.github.io/lampa/>

## Каталог

Источник истины — [`config/plugins.yml`](./config/plugins.yml). Для Lampa публикуется сгенерированный [`app/extensions.json`](./app/extensions.json). Лендинг `index.html` — статический, карточки плагинов добавляются руками.
...
```
app/
├── store.js              # главный точка входа (URL выше)
├── extensions.json       # сгенерированный каталог для Lampa
...
Личный дев-инструментарий (gitignored): `scripts/` (`build-catalog.py`, `lampa-dev.sh`, `inject-session.mjs`), `vendor/lampa-source/`, `storage/dev-session.json`, `.chrome-profile/`, `.claude/`, `openspec/`, `CLAUDE.md`.

## Добавить новый плагин

1. Положи файл `app/plugins/<name>.js` (ES5 IIFE; паттерн — в skill `lampa-plugin-development`).
2. В Claude Code запусти `/build-plugin <name>` — он спросит алиасы/категорию, зарегистрирует в `config/plugins.yml`, перегенерирует `app/extensions.json`, закоммитит и (по подтверждению) запушит.
3. После push плагин виден в сторе на ТВ при следующем открытии настроек расширений — никаких переустановок.

Вручную: правишь `config/plugins.yml`, запускаешь `make catalog`, коммитишь `config/plugins.yml` + `app/extensions.json` + `app/plugins/<name>.js` одним коммитом.
...
make catalog         # перегенерировать app/extensions.json
